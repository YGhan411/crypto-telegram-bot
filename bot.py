...
async def scalp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        equal_liquidity = detect_equal_high_low(candles)
        sweep_v3_score = 0

        if (
            equal_liquidity["type"] == "🟢 Equal Low Liquidity"
            and liquidity_v2["type"] == "🟢 Sell-side Sweep V2"
        ):
            sweep_v3_score = 3

        elif (
            equal_liquidity["type"] == "🔴 Equal High Liquidity"
            and liquidity_v2["type"] == "🔴 Buy-side Sweep V2"
        ):
            sweep_v3_score = 3
        ...
        if sweep_v3_score == 3:
            score += 2
            reasons.append("Gerçek likidite süpürmesi doğrulandı")
        if sweep_v3_score == 3:
                    ict_score += 2
        ...
        text = (
            ...
            f"🌊 Sweep V3 Score: {sweep_v3_score}/3\n\n"
            f"〰️ Equal High/Low: {equal_liquidity['type']}\n\n"
            f"💰 Fiyat: ${current_price:,.4f}\n"
